...
namespace Bank.Api.Banks;

[ApiController]
[Route("banks")]
public class BanksController : ControllerBase
{
    private readonly Money.Bank _bank = new();
    ...
    private static Money.Money From(MoneyDto dto) =>
        dto.Currency switch
        {
            "USD" => Money.Money.Dollar(dto.Amount),
            "CHF" => Money.Money.Franc(dto.Amount),
            _ => throw new ArgumentOutOfRangeException(nameof(dto))
        };

    private static MoneyDto ToDto(Money.Money money) => new() { Amount = money.Amount, Currency = money.Currency };
}